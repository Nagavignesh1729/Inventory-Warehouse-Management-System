...
async function listItems(req, res) {
  const { warehouse_id } = req.query; // Check for warehouse_id filter
  
  // Handle the special /low-stock route
  ...
  // Handle general item listing with optional filtering
  const { data, error: svcErr } = await itemService.listItems({ warehouse_id });
